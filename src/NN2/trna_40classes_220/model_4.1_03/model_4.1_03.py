...
data_train = 'YC.Bio/src/NN2/trna_40classes_220/data/train.csv'
data_valid = 'YC.Bio/src/NN2/trna_40classes_220/data/valid.csv'
weights = 'model_084_9728(base).h5'
weights2 = 'model_04_98(base2).h5'
types = 'YC.Bio/src/NN2/trna_40classes_220/data/types.txt'

with open(types, 'r') as inp:
    class_info = inp.read().split('\n')
    class_num = len(class_info)


model = Sequential()

model.add(Dropout(0.1 , input_shape=(arr_length,)))
...
model2.add(Dropout(0.1, input_shape=(input_length,)))
...
model3.add(Dropout(0.10, input_shape=(arr_length,)))
...
def get_flag(ln):
    cls = ln[-1]
    flag = [0 for i in range(class_num)]
    pos = class_info.index(cls)
    flag[pos] = 1
    ...
    while 1:
       with open(path) as f:
            ...
            for ln in r:
                ...
                X = np.concatenate([np.array(ln[1:len(ln)-1],dtype=np.uint32),np.zeros((221-len(ln),), dtype=np.uint32)])
                y = get_flag(ln)
...
csv_logger = CSVLogger('model_4.1_03.log')
...
model4.save_weights('model_4.1_03.h5')
